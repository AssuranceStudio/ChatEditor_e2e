...
Validate "MultiChat 15 Adolescent PCS" vocabulary
    ...
    Select TouchChatEnglishSimbolstix&PCS vocabularies    ${multiChat15AdolescentPCS}

Assertions for "MultiChat 15 Adolescent PCS" vocabulary
    # Click on a button's text - MultiChat 15 Adolescent PCS
    Set Resolution    ${window}    1800    800
    ${text}=    Set Variable    Questions
    Click On Button With Text    ${text}    multiChat15AdolescentPCS\\questions.png    0.6

    Check to see if the user exist

    # Check to see if the name of the vocabulary is included in the text box - MultiChat 15 Adolescent PCS
    RPA.Windows.Click    ${edit_mode_btn}
    ${message}=    RPA.Windows.Get Text    id:65535
    Should Contain    ${message}    The vocabulary "MultiChat 15 Adolescent PCS"
    RPA.Windows.Click    id:7

    # Check to see if the vocabulary 'name' attribute is its actual name - MultiChat 15 Adolescent PCS
